...
# si le paso un parametro, muestra mas detalles 
$muestradetalles = ($args.Count -gt 0)
...
if($muestradetalles) { Write-Host "Location Services: $valor" }

# Si no estaba habilitado, se habilita
if ($valor -ne "Allow") {
    New-Item -Path $path -Force | Out-Null
    Set-ItemProperty -Path $path -Name "Value" -Value "Allow"
    if($muestradetalles) { Write-Host "Location Services no estaba habilitado y se habilito" }
}
...
while ($watcher.Status -ne "Ready" -and $elapsed -lt $timeout) {
    if($muestradetalles) { Write-Host "Esperando que servicio location este listo. (Watcher.Status: $($watcher.Status) )" }
    Start-Sleep -Seconds 1
    $elapsed++
}

if ($watcher.Status -eq "Ready") {
    if($muestradetalles) { Write-Host "Servicio location (Watcher.Status: $($watcher.Status))" }
